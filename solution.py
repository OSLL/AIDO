...
class PytorchRLTemplateAgent:
    ...
    def init(self, context: Context):
        ...
        #self.model = ParamsNet('cpu')
        self.model = ParallelNet('cpu')
        #self.model.load_state_dict(torch.load('./learning/best.pkl', map_location=torch.device('cpu')))
        self.model.load_state_dict(torch.load('./ml/best.pkl', map_location=torch.device('cpu')))
    ...
    def compute_action(self, observation):
        ...
        #val = self.model.forward(obs)
        #action = [val[0][0].item(), val[0][1].item()]
        t = time.time()
        d, phi = self.model.forward(obs)
